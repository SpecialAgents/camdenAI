...
const FloatingEmojis: React.FC<{ results: SentimentResult[] }> = ({ results }) => {
  const emojis = useMemo(() => {
    // If no results, show a default mixed set
    if (results.length === 0) {
      return EMOJI_MAP.MIXED.map((char, i) => ({
        id: `default-${i}`,
        char,
        left: `${(i * 13) % 90 + 5}%`,
        top: `${(i * 17) % 90 + 5}%`,
      ...
      {emojis.map((e) => (
        <span
          ...
          style={{
            ...
            fontSize: e.size,
            zIndex: e.id.includes('default') ? -2 : -1
          }}
        >
          {e.char}
        </span>
...
const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col transition-colors duration-500">
      ...
      {/* Navigation Bar */}
      {currentView !== 'LANDING' && (
        <nav className="sticky top-0 z-50 glass-card px-6 py-4 flex justify-between items-center border-b border-[#B6C99C]/30 shadow-sm transition-all">
          <div 
      ...
      <footer className="bg-white/50 dark:bg-[#1A1C18]/50 border-t border-[#B6C99C]/20 py-8 px-6 mt-auto transition-colors">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="text-[#88976C] dark:text-[#B6C99C]/60 text-sm font-medium">
            © 2024 Camden Intelligence AI. Built for Precision & Insight.
          </div>
          <div className="flex gap-4">
            <div className="w-8 h-8 rounded-full bg-[#E8F4DC] dark:bg-[#2A2D26] flex items-center justify-center text-[#728156] dark:text-[#B6C99C]">
